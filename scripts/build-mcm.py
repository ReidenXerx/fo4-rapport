...
shows as the default). The defaults are READ from scoring.json, barks.json, narrator.json
and names.json, never
...
anatomy = json.loads((DATA / "anatomy.json").read_text(encoding="utf-8"))
...
PAGES = [
    ...
    ("Narrator", [
        ...
        ("Narrator", "addonLines", narrator, "Addon moments", "What a mod built on Rapport has to say in its own "
         "words: how a conversation went, and why.", None, None, None),
        ("section", "Names"),
        ("Names", "enabled", names, "Names for the nameless", "Someone with no name of their own - a settler, a "
         "drifter - gets one the first time a mod introduces them, and keeps it.", None, None, None),
        ("section", "History"),
    ...
    # Anatomy's cbp.dll (fo4-anatomy): the owner's "one place" for face and body knobs, 2026-09-25.
    # Rapport sends these as 'RFAK'; with Anatomy not installed they do nothing.
    ("Bodies & faces", [
        ("text", "Anatomy's body and face work, set here. These reach Anatomy's plugin by message; without "
                 "Anatomy installed they change nothing."),
        ("section", "What it does"),
        ("Anatomy", "aim", anatomy, "Aim", "The penis finds the opening it is meant for.", None, None, None),
        ("Anatomy", "shape", anatomy, "Shape", "A slimmer shaft and a fuller head, per man.", None, None, None),
        ("Anatomy", "lipFit", anatomy, "Lips fit", "Lips close around what is between them.", None, None, None),
        ("Anatomy", "faceReaction", anatomy, "Face reacts during oral", "Brows, cheeks and nose react while "
         "something is in the mouth.", None, None, None),
        ("Anatomy", "deepFace", anatomy, "Deeper, stronger face", "The face moves toward its deep version as the "
         "penis goes deeper - in the mouth, the vagina or the anus.", None, None, None),
        ("section", "Tuning"),
        ("Anatomy", "lipClearance", anatomy, "Lip clearance", "Room left between the lips and the shaft.",
         0.0, 0.2, 0.01),
        ("Anatomy", "lipSpeed", anatomy, "Lip speed", "How fast the lips follow (1 = the default rate).",
         0.25, 3.0, 0.05),
        ("Anatomy", "shaftScale", anatomy, "Shaft thickness", "Below about 0.7 the aim's collision gets thin.",
         0.6, 1.2, 0.01),
        ("Anatomy", "headMin", anatomy, "Head size, smallest", "Each man's head size is chosen between these two.",
         1.0, 2.0, 0.05),
        ("Anatomy", "headMax", anatomy, "Head size, largest", "If this is below the smallest, the smallest is "
...
lines = ["; GENERATED by scripts/build-mcm.py from scoring.json, barks.json, narrator.json, names.json and anatomy.json - edit "
         "those, not this."]
