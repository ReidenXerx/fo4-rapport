...
12. **What Overture's methodology asks of the core** (proposed 2026-09-23, overnight, nothing built;
    ...
    - **BUILT 2026-09-23, not yet run with a player scene.** **Skip barks for the player.** `Barks::OnSceneStarted` picks a line for BOTH participants by persona
    ...
    - **APPROVED by the owner 2026-09-23 (Overture O-16); BUILT the same day as `ReservePlayerScene`, not yet run.** **A priority lane for a deliberate player request.** Rapport runs one scene at a time, and Chemistry
